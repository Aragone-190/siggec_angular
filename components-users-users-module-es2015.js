...
let UsersComponent = class UsersComponent {
    constructor(permissionsService, modalService, rolesServices, homeServices, userService, formBuilder, toastr, datepipe, http) {
    ...
    ngOnInit() {
        ...
        setTimeout(() => {
            this.Logs('usuarios', 'Consulto modulo usuarios', this.currentUserSubject.value.id_usuario, 'Consulta', 'Consulto el modulo de usuarios: ');
        }, 4000);
    }
    getIPAddress() {
    ...
    showModalAgregarUsuario() {
        setTimeout(() => {
            this.Logs('usuarios', 'Consulta la actividad agregar usuario' + 'null', 'null', 'Consulta', 'Visualizo los detalles de creacion de usuario ');
        }, 1000);
    ...
    onSubmitAgregarUsuario() {
        ...
        let user = new _models_user__WEBPACK_IMPORTED_MODULE_8__["User"](null, this.userAgregarForm.get('nombre_empleado').value, this.userAgregarForm.get('nombre_usuario').value, this.userAgregarForm.get('contrasena').value, this.userAgregarForm.get('rol').value, null, this.userAgregarForm.get('cargo').value);
        this.userService.addUser(user).subscribe(result => {
            ...
            this.users.push(new _models_user__WEBPACK_IMPORTED_MODULE_8__["User"](result.id_usuario, result.nombre_empleado, result.nombre_usuario, result.contrasena, null, null, null, null, null, null, null, role.nombre_role, result.cargo));
            this.users = [...this.users];
            this.isConfirmLoadingAgregarUsuario = false;
            this.isVisibleAgregarUsuario = false;
            this.toastr.success('Usuario Agregado!');
            setTimeout(() => {
                this.Logs('usuarios', 'Creacion de un nuevo usuario: ' + result.nombre_usuario, result.nombre_empleado, 'Creacion', 'Creacion de un nuevo usuario: ' + this.userAgregarForm.get('nombre_usuario').value + ' ');
            }, 1000);
        }, err => {
            this.isConfirmLoadingAgregarUsuario = false;
    ...
    showModalEditarUsuario(data) {
        ...
        setTimeout(() => {
            this.Logs('usuarios', 'Consulta actividad de edicion usuario' + this.userDataSelected.nombre_usuario, this.userDataSelected.nombre_empleado, 'Consulta', 'Visualizo los detalles de edicion de usuario ');
        }, 1000);
    ...
    onSubmitEditarUsuario() {
        ...
        let user = new _models_user__WEBPACK_IMPORTED_MODULE_8__["User"](this.userDataSelected.id_usuario, this.userEditarForm.get('nombre_empleado').value, this.userEditarForm.get('nombre_usuario').value, this.userEditarForm.get('contrasena').value, this.userEditarForm.get('rol').value, this.userEditarForm.get('cargo').value);
        this.userService.updateUsers(user)
            ...
            setTimeout(() => {
                this.Logs('usuarios', 'Edicion del usuario ' + this.userDataSelected.nombre_usuario, this.userDataSelected.nombre_empleado, 'Actualizacion', 'Realizo cambios en el campo de nombre_empleado de "' + this.userDataSelected.nombre_empleado + '" a "' + this.userEditarForm.get('nombre_empleado').value +
                    '" y realizo cambios en nombre_usuario de "' + this.userDataSelected.nombre_usuario + '" a "' + this.userEditarForm.get('nombre_usuario').value + '"' +
                    '" y realizo cambios en contrasena de "' + this.userDataSelected.contrasena + '" a "' + this.userEditarForm.get('contrasena').value + '"' +
                    '" y realizo cambios en rol de "' + this.userDataSelected.id_role + '" a "' + this.userEditarForm.get('rol').value + '"' +
    ...
    showDeleteConfirm(data) {
        ...
        this.modalService.confirm({
            ...
            nzOkType: 'danger',
            nzOnOk: () => {
                this.userService.deleteUser(this.userDataSelected.id_usuario).subscribe(result => {
                    this.toastr.success('Usuario eliminado correctamente!');
                    setTimeout(() => {
                        this.Logs('usuarios', 'Eliminacion del usuario ' + this.userDataSelected.nombre_usuario, this.userDataSelected.nombre_empleado, 'Eliminacion', 'Se elimino el usuario: ' + this.userDataSelected.nombre_usuario + ' ');
                    }, 1000);
...
class AdmLogs {
    constructor(nombre_tabla, nombre_objeto, id_objeto, tipo_transaccion, descripcion, nombre, usuario, publicIP, privateIP, navegator, fecha_creacion) {
...
let HomeService = class HomeService {
    ...
    totalUsuarios() {
        return this.http.get(`${_environments_environment__WEBPACK_IMPORTED_MODULE_2__["environment"].url_api}usuarios/count/totales`).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])(data => {
            return data;
        }));
    }
    addLogLogin(datos) {
        ...
        console.log(data);
        return this.http.post(`${_environments_environment__WEBPACK_IMPORTED_MODULE_2__["environment"].url_api}addLog`, data).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])(result => {
