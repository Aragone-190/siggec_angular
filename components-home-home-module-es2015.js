...
let HomeComponent = class HomeComponent {
    constructor(services, peritosService, carpetasService, userService, usuariosService, indiciosService, datepipe, http, homeServices) {
        ...
        this.carpetasService.getTotal().subscribe(request => {
            this.countAnimation('totalCarpetas', request);
            if (request == 0) {
                this.countAnimatedIsCeroTotalCarpetas = true;
            }
        }, error => {
            console.log(error);
        });
        this.userService.getCountUsers().subscribe(request => {
            this.countAnimation('totalUsuarios', request);
        }, error => {
            console.log(error);
        });
        this.usuariosService.getCountUserApp().subscribe(request => {
            this.countAnimation('totalUsuariosA', request);
        }, error => {
            console.log(error);
        });
        this.indiciosService.getIndiciosTotal().subscribe(request => {
            this.countAnimation('totalIndicios', request);
            if (request == 0) {
                this.countAnimatedIsCeroTotalIndicios = true;
            }
        }, error => {
            console.log(error);
        });
        this.indiciosService.getIndiciosTotalPerito(this.currentUserSubject.value.nombre_usuario).subscribe(request => {
            this.countAnimation('totalIndicioPerito', request);
            if (request == 0) {
                this.countAnimatedIsCeroTotalIndiciosPerito = true;
            }
        }, error => {
            console.log(error);
        });
        //peritos 
        this.peritosService.getTotalFolderxPerito(this.currentUserSubject.value.nombre_usuario).subscribe(request => {
            this.countAnimation('totalCarpetasPeritos', request.length);
            if (request == 0) {
                this.countAnimatedIsCeroTotalCarpetasPeritos = true;
            }
        }, error => {
            console.log(error);
        });
    ...
    Logs() {
        ...
        let metaData = new src_app_models_logs__WEBPACK_IMPORTED_MODULE_5__["AdmLogs"]('usuarios', 'Inicio de sesion', this.currentUserSubject.value.id_usuario, 'Consulta', descripcion, this.currentUserSubject.value.nombre_empleado, this.currentUserSubject.value.nombre_usuario, this.ipAddressPublic, this.ipAddressPrivate, navegadorUsed, this.dateLogin);
        this.homeServices.addLogLogin(metaData).subscribe(result => {
        }, err => {
            console.log(err);
        });
    }
    countAnimation(tipo, total) {
        setTimeout(() => {
            let countStop = setInterval(() => {
                if (total - this.animacionContadores[tipo] > 100000) {
                    this.animacionContadores[tipo] += 100000;
                }
                else if (total - this.animacionContadores[tipo] > 50000) {
                    this.animacionContadores[tipo] += 50000;
                }
                else if (total - this.animacionContadores[tipo] > 10000) {
                    this.animacionContadores[tipo] += 10000;
                }
                else if (total - this.animacionContadores[tipo] > 1000) {
                    this.animacionContadores[tipo] += 1000;
                }
                else if (total - this.animacionContadores[tipo] > 100) {
                    this.animacionContadores[tipo] += 100;
                }
                else {
                    this.animacionContadores[tipo]++;
                }
                if (this.animacionContadores[tipo] == total) {
...
let vehiculoService = class vehiculoService {
    ...
    getRegistrosVehiculo() {
        return this.http.get(`${_environments_environment__WEBPACK_IMPORTED_MODULE_1__["environment"].url_api}getRegistrosVehiculo?page=0&size=100`).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])(result => {
            return JSON.parse(JSON.stringify(result.registers)).map(item => {
    ...
    addVehiculo(datos) {
        ...
        console.log(data);
        return this.http.post(`${_environments_environment__WEBPACK_IMPORTED_MODULE_1__["environment"].url_api}addNewVehiculo`, data).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])(result => {
